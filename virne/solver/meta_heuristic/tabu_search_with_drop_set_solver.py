...
import threading
import numpy as np
import virne.solver.meta_heuristic.drop_set as ds
...
class TabuSearchWithDropSetSolver(MetaHeuristicSolver):
    """
    Tabu Search with Drop Set (tsds) for VNE

    References:
        - TODO et al. "Virtual Network Forwarding Graph Embedding Based on Tabu Search with Drop-Set". TODO, 2019.
    ...
    def __init__(self, controller: Controller, recorder: Recorder, counter: Counter, **kwargs):
        super(TabuSearchWithDropSetSolver, self).__init__(controller, recorder, counter, **kwargs)
        """
        """
        # super parameters
    ...
    def evolve(self, individual):
        ...
        while iter_id < self.max_iteration:
            ...
            if individual.solution not in drop_set_results.values() and diff_fitness < 0:
                individual.last_solution = copy.deepcopy(individual.solution)
                individual.update_best_solution()
                self.drop_set.add_item(individual.last_solution, drop_set_results)
            iter_id += 1
