...
class TabuSearchSolver(MetaHeuristicSolver):
    """
    Tabu Search (ts) for VNE

    References:
        - TODO et al. "Virtual Network Forwarding Graph Embedding Based on Tabu Search". TODO, 2019.
    ...
    def __init__(self, controller: Controller, recorder: Recorder, counter: Counter, **kwargs):
        super(TabuSearchSolver, self).__init__(controller, recorder, counter, **kwargs)
        """
        """
        # super parameters
    ...
    def evolve(self, individual):
        ...
        while iter_id < self.max_iteration:
            ...
            if individual.solution not in self.tabu_list and diff_fitness < 0:
                individual.last_solution = copy.deepcopy(individual.solution)
                individual.update_best_solution()
